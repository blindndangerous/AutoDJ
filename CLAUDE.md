...
- [x] **Voice-liner options collapse with master toggle** — master "Enable voice liners" checkbox lifted out of the Trigger fieldset and given `aria-describedby="ln-enabled-desc"`; the Trigger / Mix / Library fieldsets now sit behind a single `data-show-when="ln-enabled"` so the entire option set hides as one group when the feature is off (no orphan legend, no orphan rows).  Master toggle + description remain visible so users always know how to turn the feature back on
