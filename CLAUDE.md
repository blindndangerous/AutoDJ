...
- **FAISS file rewrite throttled to every `FAISS_CHECKPOINT_EVERY = 100` tracks** (or the final track of the run).  The monolithic `vectors.index` is ~290 MB at 70 k tracks × 1040 dims × fp32; rewriting it per-track was the last big NAS-write hammer.  Metadata is still durable per-track via `tracks.db` UPSERT.  On startup, `_load_existing_index` reconciles any mismatch: if a crash left `tracks.db` ahead of `vectors.index`, the trailing rows are trimmed back so those tracks get re-embedded on the next run (max loss = `FAISS_CHECKPOINT_EVERY - 1` tracks of FAISS work).
